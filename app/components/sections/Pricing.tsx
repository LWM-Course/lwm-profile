...
interface PricingProps {
  initialJenjang?: keyof typeof pricingData;
  hideTabs?: boolean;
}

export const Pricing: React.FC<PricingProps> = ({ initialJenjang = 'SD', hideTabs = false }) => {
  const [activeTab, setActiveTab] = useState<keyof typeof pricingData>(initialJenjang);
    ...
    <Section id="jenjang" className="bg-white">
      ...
      {!hideTabs && (
        <div className="flex justify-center mb-16">
          <div className="bg-orange-50 p-1.5 rounded-full inline-flex shadow-inner">
            {(Object.keys(pricingData) as Array<keyof typeof pricingData>).map((level) => (
              <button
                key={level}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {pricingData[activeTab].map((plan, index) => (
          <Card 
            key={`${activeTab}-${index}`} 
            overflow="visible"
            className={`relative h-full flex flex-col hover:shadow-xl transition-all duration-300 border-2 ${index === 1 ? 'border-brand-yellow z-10' : 'border-transparent hover:border-orange-100'}`}
          >
            {index === 1 && (
            ...
            <Button variant={index === 1 ? 'primary' : 'outline'} fullWidth>
              Pilih Paket
            </Button>
          </Card>
        ))}
